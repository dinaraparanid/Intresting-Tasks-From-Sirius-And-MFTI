...
class SegmentTree
{
 private:
	...
	static inline int left(int _p) { return _p << 1; } // левая ветвь
	static inline int right(int _p) { return (_p << 1) + 1; }  // правая ветвь
	...
	*/
	...
	void build (int _p, int _l, int _r)
	{
		...
		else
		{
			...
			const int p1 = builder_[left(_p)];
			const int p2 = builder_[right(_p)];
	...
	*/
	...
	int rmq (int _p, int _l, int _r, int _i, int _j)
	{
		...
		const int p1 = rmq(left(_p), _l, (_l + _r) / 2, _i, _j);
		const int p2 = rmq(right(_p), (_l + _r) / 2 + 1, _r, _i, _j);
 ...
 public:
	...
	*/
	...
	explicit SegmentTree(const std::vector<int>& _vec/*, STOrder _order*/)
	{
		constructor_ = _vec;
		last_ = _vec.size();
		//order_ = _order;
	...
	*/
	...
	inline void rebuild (const std::vector<int>& _vec)
	{
		constructor_ = _vec;
		std::for_each(builder_.begin(), builder_.end(), [](int& _elem) { _elem = 0; });
		build(1, 0, last_ - 1);
	}

	inline int rmq (int _i, int _j) { return rmq(1, 0, last_ - 1, _i, _j); } // перегрузка для нахождения
